...
variable_list = list(K.reshape(1, -1).squeeze())
...
model.compile("adam", lr=1e-4, external_trainable_variables=[K])
...
for dim in range(N_DIMS):
    ax[dim].plot(t, u[:, dim], label="Solution", color='gray')
    if dim in sensor_indices:
        ax[dim].plot(tdata, udata[:, dim], label="Data", linestyle="None", marker=".", color='green')
    ax[dim].plot(t, upred[:, dim], label="Prediction", color='black')
    ax[dim].set_xlabel(r"$t$")
    ax[dim].set_ylabel(r"$u_{}(t)$".format(dim + 1))
    if dim == 0:
        ax[dim].legend(loc="upper right", ncol=2)
